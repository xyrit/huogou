...
class CaptchaAction extends Action
{
    ...
    public function getVerifyCode($regenerate = false)
    {
        ...
        $cache = Yii::$app->cache;
        $name = $this->getCaptchaKey();
        if (!$cache->get($name) || $regenerate) {
            $cache->set($name, $this->generateVerifyCode(), 1800);
            $cache->set($name . 'count', 1, 1800);
        }
    ...
    {
        if ($this->getCaptchaKey() == md5('__captcha/')) {
            return false;
        }
        $code = $this->getVerifyCode();
        $valid = $caseSensitive ? ($input === $code) : strcasecmp($input, $code) === 0;
        $cache = Yii::$app->cache;
        $name = $this->getCaptchaKey() . 'count';
        $cache->set($name, $cache->get($name)+1, 1800);
        if ($cache->get($name) > $this->testLimit && $this->testLimit > 0) {
...
//        $vowels = 'aeiou';
     ...
     */
    protected function getCaptchaKey()
    {
    ...
    {
        return $this->renderImageVcode($code);
...
//        }
    ...
    protected function renderImageVcode($code)
    {
        ...
        imagefilledrectangle($img, 0, 0, $this->width, $this->height, $backColor);
        imagecolordeallocate($img, $backColor);
        ...
        $_x = $this->width / $this->maxLength;
        $fontSize = (int) ($_x * 0.85);
        for ($i=0;$i<$this->maxLength;$i++) {
            $fontcolor = imagecolorallocate($img,mt_rand(0,156),mt_rand(0,156),mt_rand(0,156));
            imagettftext($img,$fontSize,mt_rand(-30,30),$_x*$i+mt_rand(1,5),$this->height / 1.4,$fontcolor,$this->fontFile,$code[$i]);
        }
    ...
    protected function renderImageByGD($code)
    {
        ...
        for ($i = 0; $i < $length; ++$i) {
            $fontSize = (int) (rand(26, 32) * $scale * 0.7);
            $angle = rand(-10, 10);
            $letter = $code[$i];
...
//        $distortionImage = imagecreatetruecolor($this->width, $this->height);
//        imagefill($distortionImage, 0, 0, 0xFFFFFF);
//        for ($i = 0; $i < $this->width; $i++) {
//            for ($j = 0; $j < $this->height; $j++) {
//                $rgbColor = imagecolorat($image, $i, $j);
//                imagesetpixel($distortionImage, (int) ($i + sin($j / $this->height * 2 * M_PI - M_PI * 0.5) * 5), $j, $rgbColor);
//            }
//        }
//        $image = $distortionImage;

//        for ($i = 0; $i < 5; $i++) {
//            $lineColor = imagecolorallocate($image, mt_rand(0, 255), mt_rand(0, 255), 20);
//            imageline($image, 0, mt_rand(0, $this->width), $this->width, mt_rand(0, $this->height), $lineColor);
//        }

...
class Purchase extends \yii\db\ActiveRecord
{
    ...
    public static function getList($where,$page,$perpage,$orderBy = ' create_time desc '){
        $query = Purchase::find()->select('p.*,admin.username,vpo.status as order_status')->from(static::tableName().' p')->leftJoin('admin',' p.admin_id = admin.id')->leftJoin('virtual_purchase_order vpo','vpo.purchaseid=p.id');
        if ($where) {
            $query->where($where);
        }

...
class MemberController extends BaseController
{
    ...
    public function actionPostone()
    {
        ...
        if ($request->isPost) {
            ...
            $result = ShareTopic::updateTopic($model, $params);
            if ($result) {
                Order::updateAll(['status' => 8], ['id' => $id]);
            }
            return Json::encode(['flag' => $result]);
        }
    ...
    public function actionMobilecheck()
    {
        ...
        if (empty($userInfo['phone'])) {
            return $this->redirect(['/mobile/member/mobilebind']);
        }
        $mobile = User::privatePhone($userInfo['phone']);
    ...
    public function actionEmailcheck()
    {
        ...
        if (empty($userInfo['email'])) {
            return $this->redirect(['/mobile/member/emailbind']);
        }

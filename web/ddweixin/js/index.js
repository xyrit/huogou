...
$(function () {
    ...
    //首页轮播图
    $.getJsonp(apiBaseUrl + '/banner/banner-list', {type: 0, source: 1, num: 5}, function (json) {
...
function createGoodsListHtml(json, append) {
    ...
    $.each(json.list, function (i, v) {
        ...
        html += '<a href="javascript:;" class="buy-btn" periodId="' + periodId + '"  buyUnit="'+buyUnit+'">立即夺宝</a>';
...
function createCateListHtml(json) {
    ...
    $('.select-total ul li').on('click', function() {
        ...
        var buyUnit = $(this).attr('buyUnit');
        var url = weixinBaseUrl+'/list.html?order='+orderFlag+'&cid='+cid;
        if (limitFlag) {

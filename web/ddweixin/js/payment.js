...
function createGoodsListHtml(json) {
    ...
    $('.g-pay-lst .orange').html('<i></i>' + totalMoney + '夺宝币</em>');
}
...
function renderPayInfo() {
    $.getJsonp(apiBaseUrl + '/user/getmoney', {}, function (json) {
        ...
        $('.other_pay a:eq(1)').html('<i></i>余额支付：<span class="gray9">(余额' + money + '夺宝币)</span><em class="orange fr"></em>');
...
function renderPaypointHtml(payPoints) {
    if (payPoints > 0) {
        ...
        $('.other_pay a:eq(0) .orange').html('-<b></b>' + (payPoints / 100) + '夺宝币');
    } else {
...
function renderPaymoneyHtml(payMoney) {
    if (payMoney > 0) {
        ...
        $('.other_pay a:eq(1) .orange').html('-<b></b>' + payMoney + '夺宝币');
    } else {
...
function renderPaybankHtml(payBank) {
    if (payBank > 0) {
        ...
        $('.other_pay a:eq(2) .orange').html('<span class="colorbbb">需要支付&nbsp;</span><b></b>' + payBank + '夺宝币');
    } else {
        $('.other_pay .net-pay').hide();

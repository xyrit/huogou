    ...
    $.getJsonp(apiBaseUrl + '/cart/list', {'check':'1',isbuy: 1}, function (json) {
        ...
        $('#btnPay').on('click', function () {
            ...
            var submitPay = function(ppwd) {
                ...
                $.post(mobileBaseUrl + '/payapi/create-order', data, function (json) {
                    ...
                    if (json.code == 100) {
                        ...
                        if (payName == 'balance') {
                            window.location.href = mobileBaseUrl + '/shopok.html?o=' + orderId;
...
function checkPpwd(payPoint,payMoney,payBank,func) {
    ...
    changePayBtnState(false,'正在输入密码...');
    ...
    var W = function () {
        ...
        if (Y.length == 6) {
            ...
            $.getJsonp(apiBaseUrl + '/pay/check-ppwd', {pwd: Y}, function (json) {
                if (json.code == 1) {
                    changePayBtnState(false,'正在提交支付...');
                    func(Y);
                } else {

...
$(function() {
    ...
    $.getJsonp(apiBaseUrl+'/period/info', {id:periodId}, function(json) {
        ...
        if (periodLeftTime>0) {
            ...
            $('.ann_btn:eq(1) a:eq(1)').attr('href', mobileBaseUrl+'/buyrecords-'+periodId+'.html');
            $('.ann_btn:eq(1) a:eq(2)').attr('href', mobileBaseUrl+'/goodsimgdesc-'+productId+'.html');
            $('.ann_btn:eq(1) a:eq(3)').attr('href', mobileBaseUrl+'/goodspost-'+productId+'.html');
            ...
            var buyrecordsUrl = mobileBaseUrl+'/buyrecords-'+periodId+'.html';
            var calresultUrl = wwwBaseUrl+'/app/compute.html?pid='+periodId;
            var goodspostUrl = mobileBaseUrl+'/goodspost-'+productId+'.html';
            var moreperiod = mobileBaseUrl+'/moreperiod-'+productId+'.html';
        ...
        if (periodInfo.current_number) {
            $(".pro_foot").show();
            //$(".conductBtn").html('<a href="/product/'+productId+'.html">第'+periodInfo.current_number+'期正在进行中…</a>');
...
function createPeriodList(json) {
    ...
    if (parseInt(curPeriodNumber)==parseInt(thsPeriodNumber)+1) {
        //html += '<li><a href="'+productUrl+'" ><s class="fl"></s>第'+curPeriodNumber+'期<i class="fr"></i> </a> </li>';
        html += '<li><a href="'+productUrl+'" ><s class="fl"></s>'+curPeriodNumber+'期<i class="fr"></i> </a> </li>';
    }
    $.each(json.list, function(i,v) {
        var periodId = v.id;
        //var periodNumber = v.period_number;
        var periodNumber = v.period_no;
        ...
        if (thsPeriodNumber==periodNumber) {
          //  html += '<li><a href="'+periodUrl+'" class="hover"><s class="fl"></s>第'+periodNumber+'期<i class="fr"></i> </a> </li>';
            html += '<li><a href="'+periodUrl+'" class="hover"><s class="fl"></s>'+periodNumber+'期<i class="fr"></i> </a> </li>';
        } else {

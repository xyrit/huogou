...
$(function() {
    $('#divBtmMoney').click(function(){
        ...
        $.getJsonp(apiBaseUrl+'/cart/check',cartData,function(json) {
            if (json.code == 100) {
                ...
                if (json.logined == 0) {
                    location.href = '/passport/login.html?forward='+encodeURIComponent(mobileBaseUrl+'/cart.html');
                }
            }
        });
        return;
    ...
    //nav固定
    var navTop = $('.divDownApp2').offset().top;
    var navOff = 0;
    $(window).on('scroll',function(){
        if ($(window).scrollTop() > navTop && !navOff) {
            $('.divDownApp2').addClass('pFixed');
            navOff = 1;
        }else if($(window).scrollTop() < navTop && navOff){
            $('.divDownApp2').removeClass('pFixed');
            navOff = 0;
        }
    })
...
function setTotalPaymentMoney() {
    ...
    $('#divBtmMoney .orange:eq(1)').html(total+'.00');
}
...
function createCartListHtml(json) {
    ...
    $.each(list, function(i,v) {
        ...
        if (limitNum>0) {
            ...
            item += '<a class="fl u-Cart-img" href="'+goodsUrl+'"><img src="'+goodsImgUrl+'" border="0" alt="" /> <div class="pTitle pPurchase" style="display:none">限购</div></a>';
        }
        item += '<div class="u-Cart-r">';
        if (oldPeriodId!=0 && oldPeriodId!=periodId) {
            item += '<a href="'+goodsUrl+'" class="gray6"><strong>已更新至第'+periodNumber+'期</strong>'+goodsName+'</a>';
        } else {
    ...
    if (productNum>0) {
        $('#divBtmMoney').html('<p>总共伙购<span class="orange arial z-user">'+productNum+'</span>个商品  合计金额：<span class="orange arial">'+totalMoney+'.00</span> 伙购币</p><a href="javascript:;" class="orgBtn" id="a_payment">结 算</a>');
    }else{
        $("#divBtmMoney").html('');
    }
}

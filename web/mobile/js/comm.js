...
var mobileBaseUrl = 'http://m.' + baseHost;
...
jQuery.extend({
    getJsonp: function(url, data, succfunc) {
        $.jsonp({
            ...
            success: function(json){succfunc(json);
                if(json.totalCount == "0")
                {
                    var html = '<div class="noRecords colorbbb clearfix" id="nohave"><s></s>最近三个月无记录 <div class="z-use">请使用电脑访问www.huogou.com查看更多</div> </div>';
                    var e = $("#divPostList,.goodList");
                    $(e).html(html);
                }
            },
...
function createGoodsUrl(productId) {
    return mobileBaseUrl+'/product/'+productId+'.html';
}

function createPeriodUrl(periodId) {
    return mobileBaseUrl+'/lottery/'+periodId+'.html';
}
function createUserCenterUrl(homeId) {
    return mobileBaseUrl+'/userpage/'+homeId;
}

function createPostDetailUrl(postId) {
    return mobileBaseUrl+'/post/detail-'+postId+'.html';
}
...
function goCart(periodId, num, productId) {
    ...
    $.getJsonp(url, data, function(json) {
        var code = json.code;
        if (code == 100) {
            location.href = mobileBaseUrl+'/cart.html';
        } else if (code == 101) {//已满员被抢光
            $.PageDialog.confirm('本期已满员，是否伙购下一期？',function() {
...
function returnBtn() {
    ...
    var e = $(".g-header h2");
    if(e.length && !e.text() && ! title.match("伙购网")) 
    {
       e.html(title);
       document.title = title+"_伙购网";
    }
}
   ...
   $("input:password").bind("copy cut paste",function(e){
    ...
    if (hash!='#nofastnav') {
        if ($('.footer').length==0) {
            ...
            fastNav(3);
            $('#pageBottom_footer').css('padding-bottom','60px');
        }
    }
    returnBtn();
